...
export default function HospitalAddPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/users", { headers: { Authorization: `Bearer ${token}` } })
      .then(res => res.json())
      .then((data: { users: { id: number; name: string; email: string; role: string }[] }) => setAdmins((data.users || []).filter((u: { id: number; name: string; email: string; role: string }) => u.role === "hospitalAdmin")));
  }, []);
